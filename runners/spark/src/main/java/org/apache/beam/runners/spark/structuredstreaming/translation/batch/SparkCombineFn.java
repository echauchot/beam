...
public class SparkCombineFn<InputT, ValueT, AccumT, OutputT>
    extends Aggregator<
        ...
        SparkCombineFn.WindowedAccumulator<InputT, ValueT, AccumT, ?>,
        Iterable<WindowedValue<OutputT>>> {
  private final boolean globalCombine;
  private final SerializablePipelineOptions options;
  private final Map<PCollectionView<?>, WindowingStrategy<?, ?>> sideInputStrategies;
  private final SideInputBroadcast sideInputBroadcast;
  ...
  SparkCombineFn(
    ...
    this.sideInputStrategies = sideInputStrategies;
    this.sideInputBroadcast = sideInputBroadcast;
  ...
  static <K, V, AccumT, OutputT> SparkCombineFn<KV<K, V>, V, AccumT, OutputT> keyed(
    ...
    return new SparkCombineFn<>(
        ...
        sideInputBroadcast,
        windowingStrategy,
        nonMergingStrategy, accumulatorCoder, outputCoder);
  }
  ...
  @Override
  public WindowedAccumulator<InputT, ValueT, AccumT, ?> zero() {
    return WindowedAccumulator.create(this, toValue, windowingStrategy, windowComparator);
  }

  @Override
  public WindowedAccumulator<InputT, ValueT, AccumT, ?> reduce(
  ...
  @Override
  @SuppressWarnings({"unchecked", "rawtypes"})
  public WindowedAccumulator<InputT, ValueT, AccumT, ?> merge(
  ...
  @Override
  public Iterable<WindowedValue<OutputT>> finish(
  ...
  @Override
  public Encoder<WindowedAccumulator<InputT, ValueT, AccumT, ?>> bufferEncoder() {
  ...
  private WindowedAccumulator.Type getType(WindowingStrategy<?, ?> windowingStrategy) {
    if (windowingStrategy.getWindowFn().isNonMerging()) {
      ...
      if (windowingStrategy.getWindowFn().assignsToOneWindow()
          && WindowingHelpers.isEligibleForGroupByWindow(windowingStrategy)) {
  ...
  SparkCombineContext ctxtForWindows(Collection<BoundedWindow> windows) {
    if (combineContext == null) {
      combineContext =
          new SparkCombineContext(
              options.get(), new SparkSideInputReader(sideInputStrategies, sideInputBroadcast));
    }
